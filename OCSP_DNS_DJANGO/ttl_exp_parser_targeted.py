...
only_allowed_bind_logs = ["query.log-51", "query.log-52", "query.log"]
...
def is_allowed_bind(element):
    for e in only_allowed_bind_logs:
...
def parse_logs_together(allowed_exp_ids):
    ...
    bind_files = [bind_dir + f for f in listdir(bind_dir) if isfile(join(bind_dir, f)) and ('.gz' not in f) and is_allowed_bind(f)]
...
def master_calc(ttl_list):
    ...
    for i in range(300, 404):
        for j in range(1, 7):
            allowed_exp_id.append("live_zeus_5_{}_{}".format(i, j))
    ...
    for ttl in ttl_to_exp_id_list:
        ...
        parent_path = 'ttl_result/{}/'.format("targeted")
